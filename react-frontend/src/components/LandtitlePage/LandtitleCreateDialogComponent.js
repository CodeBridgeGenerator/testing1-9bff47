...
const LandtitleCreateDialogComponent = (props) => {
    ...
    const validate = () => {
        ...
        const error = {};
            ...
            if (_.isEmpty(_entity?.landtitle)) {
                error["landtitle"] = `Landtitle field is required`;
                ret = false;
            }
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,landtitle: _entity?.landtitle,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Landtitle created successfully" });
...
<div className="col-12 md:col-6 field mt-5">
            <span className="align-items-center">
                <label htmlFor="landtitle">Landtitle:</label>
                <InputText id="landtitle" className="w-full mb-3 p-inputtext-sm" value={_entity?.landtitle} onChange={(e) => setValByKey("landtitle", e.target.value)}  required  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["landtitle"]) ? (
              <p className="m-0" key="error-landtitle">
                {error["landtitle"]}
              </p>
            ) : null}
          </small>
